...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		outputs := make([]chan [][]uint8, threads)
		for i := 0; i < threads; i++ {
			//out <- makeMatrix((height / threads), width/threads)
			outputs[i] = make(chan [][]uint8)
		}
		for i := 0; i < threads; i++ {
			startY := height / threads * i
			endY := height / threads * (i + 1)
			go worker(startY, endY, 0, width, immutableData, outputs[i])
		}

		for i := 0; i < threads; i++ {
			//out := make([][]uint8, (height / threads), width/threads)
			out := <-outputs[i]
			newPixelData = append(newPixelData, out...)
		}
	}
